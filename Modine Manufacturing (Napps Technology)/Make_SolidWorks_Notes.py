...
from Get_SolidWorks_Files import Get_SolidWorks_Files
...
def main():
   ...
   jobInst = Get_SolidWorks_Files()
   partFolders = defaultdict(str)
   ...
   for bendFolder in bendFolders:
      ...
      parts,pipes = jobInst.getParts()
      partFolders[bendFolder] = parts
   ...
   for bendFolder in bendFolders:
      ...
      flags = makePipeNote(lengths, bendFolder, pipeFolders)
      for flag in flags:
         allFlags.append(flag)
      makePartNote(bendFolder, pipeFolders)
   loader.stop()
...
def makePipeNote(lengths, folder, pipeFolders) -> list:
   """Creates the formatted note in the job folder"""
   flags = []
...
def makePartNote(folder, pipeFolders):
   """Creates the formatted note in the job folder"""
