...
# first_day = datetime.today().replace(day=1, month=1).date()
...
def main():
   ...
   for file in list_of_files:
      ...
      if created_day <= today and created_day >= first_day:
   ...
   file = output_folder + "\\Month of " + file_day + " Log.csv"
   week_review[0].to_csv(file, sep=',', mode='a')
   with open(file,"a") as f:
      f.write(f"\nLength of Month: {day_count} Days\n")
      f.write(f"Days worked from Home: {days_home}\n")
      f.write("Total Hours: {:0.2f}\n".format(week_length))
      f.write(f"Average Start Time: {week_review[1]}\n")
      f.write(f"Average End Time: {week_review[2]}\n")
      f.write(f"Earliest Morning: {start_min}\n")
      f.write(f"Latest Night: {end_max}\n")
